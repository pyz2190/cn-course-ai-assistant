...
def test_uploaded_material_becomes_retrievable(client: TestClient) -> None:
    # 断言只针对「新导入的资料能否被检索到」，不依赖 min_retrieval_score 的取值：
    # 阈值调整会让无关旧语料以低分出现，但不应影响本用例的结论。
    before = _ask(client)
    assert "VRRP" not in before["answer"]
    ...
    assert payload["resource_id"] not in {
        citation["resource_id"] for citation in before["citations"]
    }

    after = _ask(client)
    assert after["confidence"] > 0
    assert payload["resource_id"] in {
        citation["resource_id"] for citation in after["citations"]
    }
