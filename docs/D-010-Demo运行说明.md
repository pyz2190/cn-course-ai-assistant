...
**操作流程：**
1. 点击一个**建议问题**按钮，或在输入框手动输入问题
2. 点击 **"发送"** 按钮
3. 查看生成的回答、置信度百分比、行内引用角标与引用来源列表

**功能特点：**
- 多轮对话：用户与 AI 的回答以气泡形式保留在对话流中
- 行内引用：回答文字中的 `[1]` 角标可点击，与下方引用卡片联动高亮
- 引用来源包含：资料标题、章节、页码范围、原文摘录（可展开）
...
**操作流程：**
1. 点击左侧任务卡片（每张卡片带彩色类型标签）
2. 右侧展示任务详情：任务目标、任务流程步骤、关联知识点、关联资料、完成判据、AI 反馈介入点

---

## 六、截图说明

> 以下 **8 张**核心截图展示了 Demo 的关键界面与验证结果，与「八、验收对照」逐项对应。

### 截图 1：系统主页总览

**操作：**
1. 浏览器打开 `http://localhost:5173`
2. 等待 1–2 秒，让右侧任务列表加载完成

**图中内容：**
- 顶部标题「计算机网络 AI 助教」+ 右上角绿色状态点（"API 已连接 · 离线 RAG"）
- 顶部深色提示条「离线 RAG · Qdrant 检索 · 按句引用 · 六类任务」
- 左侧聊天窗口（欢迎语 + 5 个建议问题按钮）
- 右侧六类任务卡片（带彩色类型标签）+ 默认选中的任务详情

> ![系统主页总览](image/image1.png)
...
**操作：**
1. 在左侧聊天窗口，点击一个**建议问题**（例如「TCP 为什么需要三次握手？」），或手动输入问题
2. 点击 **"发送"** 按钮
3. 等待回答出现

**图中内容：**
- 用户问题（绿色气泡，靠右）+ AI 回答（浅色气泡，靠左）
- 回答文字中带蓝色行内角标 `[1]`
- 回答下方显示「置信度 xx% · 离线检索」
- 一条引用卡片：「计算机网络：自顶向下方法 · 第 3 章 运输层 · 第 214–219 页」

> ![提问获得带引用回答](image/image2.png)
...
**图中内容：**
- 引用卡片展开，显示带左侧绿色边框的原文摘录
- 原文内容与课程 Chunk 一致

> ![展开引用原文](image/image4.png)
...
**操作：**
1. 在右侧任务列表，点击 **"Wireshark 分析 TCP 三次握手"** 任务卡片
2. 观察右侧任务详情

**图中内容：**
- 六类任务卡片，每张带不同颜色的类型标签
- 选中任务显示：类型徽章 + 任务目标 + 描述
- **任务流程**步骤（圆形序号 + 步骤名，如「阅读实验指导 → 搭建环境 → 抓包/仿真 → …」）
- 下方四个列表：关联知识点、关联资料、完成判据、AI 反馈介入点

> ![任务卡片与流程步骤](image/image5.png)
...
**图中内容：** JSON 响应 `{"status":"ok","service":"cn-course-ai-assistant-api","mode":"offline"}`

> ![API 健康检查](image/image6.png)
...
**图中内容：** FastAPI 自动生成的 Swagger 文档，可见所有端点（health / resources / qa / tasks / events）

> ![API 交互式文档](image/image7.png)

---

### 截图 8：测试通过结果

**操作：** 在项目目录运行 `npm run test`

**图中内容：**
- API 测试（pytest）109 项全部通过
- Web 测试（vitest）8 项全部通过

> ![测试通过结果](image/image8.png)
...
| 可选截图 | 操作 | 展示内容 |
|---|---|---|
| 后端启动日志 | 查看运行 `npm run dev` 的终端 | Uvicorn + Vite 启动日志 |
| 契约与构建检查 | 运行 `npm run check` | lint / typecheck / build / contracts 全通过 |

> ![npm run dev终端](image/rundev.png)

> ![npm run check终端](image/runcheck.png)
...
| 引用含资源、章节、页码 | Citation 字段完整 | 截图 2、4 |
| 引用可追溯（行内角标） | 角标与引用卡片联动 | 截图 3 |
| 六类任务均可读取 | 6 个任务卡片 | 截图 1、5 |
| 任务详情含四要素 | 知识点/资料/判据/反馈点 | 截图 5 |
| 系统状态可感知 | SystemStatus 组件 | 截图 1 |
| API 健康检查可调用 | health 端点 | 截图 6 |
| 完全离线可运行 | 无外部依赖 | 截图 6、7 |
| 测试全部通过 | pytest + vitest | 截图 8 |
