...
> 说明：以下 **8 张**核心截图是验收需要提交的内容。每张截图下方都留有【贴图】占位，截图完成后请替换占位文字，或在对应位置插入图片。
...
**操作步骤：** 浏览器打开 `http://127.0.0.1:8000/api/v1/health`

**截图应包含：** JSON 响应 `{"status":"ok","service":"cn-course-ai-assistant-api","mode":"offline"}`
...
**截图应包含：** FastAPI 自动生成的 Swagger 文档，可见所有端点（health / resources / qa / tasks / events）
