...
| `exercise_ids` | list[string] | 关联试题 ID | `["ex-transport-handshake-001"]` |
...
任务不是孤立的说明文字，它必须同时关联知识点、课程资料、试题和 AI 反馈。
四类关联各自回答一个问题，缺任何一类任务都不完整：

| 关联对象 | 字段 | 回答的问题 | 由谁解析 | 约束 |
|---|---|---|---|---|
| 知识点 | `knowledge_point_ids` | 这个任务在教什么？ | `GET /api/v1/knowledge-points` | 至少一个 |
| 课程资料 | `resource_ids` | 学生去哪里学？ | `GET /api/v1/resources` | 至少一个 |
| 试题 | `exercise_ids` | 怎么判断学生学会了？ | `GET /api/v1/exercises?task_id=` | 可为空，但六类 Demo 任务均已关联 |
| AI 反馈 | `ai_feedback_points` | 学生卡住时 AI 何时介入？ | 任务引擎内部 | 至少一条 |
...
任务 ──关联──→ 知识点 ──定位──→ 课程资料
 ...
 │            └─ 学生作答
 │                   ↓
 └──触发──→ AI 反馈（基于试题参考答案与课程资料引用给出提示）
```

**试题是完成判据从"文字描述"变成"可判定"的关键一环。**
`completion_criteria` 写的是"完成知识点自测并解释最长前缀匹配"，
但只有当它挂上一道具体的试题、试题带有 `reference_answer` 时，
"完成"才有客观依据，AI 反馈才有比对基准。

**Exercise 核心字段：**

| 字段 | 类型 | 说明 |
|---|---|---|
| `exercise_id` | string | 试题唯一标识 |
| `question` | string | 题干 |
| `exercise_type` | ExerciseType | `single_choice` / `multiple_choice` / `short_answer` / `analysis` / `design` |
| `knowledge_point_ids` | list[string] | 关联知识点，至少一个 |
| `resource_ids` | list[string] | 命题依据的课程资料 |
| `difficulty` | Difficulty | `introductory` / `intermediate` / `advanced` |
| `options` | list[string] | 选择题选项；非选择题必须为空 |
| `reference_answer` | string | 参考答案，AI 反馈的比对基准 |
| `explanation` | string | 解析，用于反馈时说明错在哪里 |
| `source` | ExerciseSource | `textbook` / `lab_guide` / `past_exam` / `course_team` |
| `review_status` | ReviewStatus | `draft` / `reviewed` / `approved` |

**取题方式：**

```bash
# 按任务取题：返回该任务 exercise_ids 关联的试题
GET /api/v1/exercises?task_id=task-foundation-addressing

# 按知识点取题：沿知识图谱取题，用于自测与复习
GET /api/v1/exercises?knowledge_point_id=kp-network-addressing
```

**题型与任务类型的对应：** 设计类任务（创新挑战、项目实践）配 `design` 题，
分析类任务（协议分析）配 `analysis` 题，基础学习任务配选择题做快速自测。
题型不是随意选的——它决定了 AI 反馈能做到多细：选择题可以精确指出
错选项对应哪个知识点，设计题只能按参考答案的要点逐条检查是否齐备。
...
| `Exercise` 试题模型 | `apps/api/app/domain/models.py` | ✅ 已实现 |
| `ExerciseRepository` 端口 | `apps/api/app/services/ports.py` | ✅ 已实现 |
| 课程试题库与仓库实现 | `apps/api/app/adapters/exercises.py` | ✅ 已实现 |
| `GET /api/v1/exercises` 路由 | `apps/api/app/api/routes/exercises.py` | ✅ 已实现 |
| 任务 → 试题关联（`exercise_ids`） | `apps/api/app/domain/models.py` | ✅ 已实现 |
| 任务模板 JSON Schema | `contracts/schemas/TaskTemplate.json` | ✅ 已实现 |
| 试题 JSON Schema | `contracts/schemas/Exercise.json` | ✅ 已实现 |
...
| 任务完成自动判定 | 基于试题参考答案自动判定作答 | 中 |
| 学生作答与判分接口 | 当前只提供取题，尚未收作答 | 中 |
| 试题来源扩展 | 接入课程题库需 Canvas Quizzes 通道，见 D-002 P5 | 低 |
...
| `POST` | `/api/v1/tasks` | 发布教学任务，可传 `status=draft` 先存草稿 |
| `PATCH` | `/api/v1/tasks/{task_id}/status` | 任务状态流转 |
| `GET` | `/api/v1/exercises?task_id=` | 获取任务关联的试题 |
| `GET` | `/api/v1/exercises?knowledge_point_id=` | 按知识点取题 |
| `POST` | `/api/v1/events` | 记录学习行为事件 |

### 9.2 任务模板 JSON Schema

详见 `contracts/schemas/TaskTemplate.json`、`contracts/schemas/Exercise.json` 和 `contracts/examples/task-template.json`。字段级接口需求见 [D-009 接口需求说明](D-009-接口需求说明.md)。
...
| task_id | 任务名 | 类型 | 关联知识点 | 关联试题 | 题型 | 核心判据 |
|---|---|---|---|---|---|---|
| `task-foundation-addressing` | 网络层寻址路径学习 | 基础学习 | kp-network-addressing | `ex-network-addressing-001` | 单选 | 完成自测并解释最长前缀匹配 |
| `task-protocol-tcp-handshake` | Wireshark 分析 TCP 三次握手 | 协议分析 | kp-transport-tcp-handshake | `ex-transport-handshake-001` | 分析 | 标注三类报文并解释序列号变化 |
| `task-case-congestion` | Reno 与 BBR 拥塞控制案例 | 案例分析 | kp-transport-congestion-control | `ex-transport-congestion-001` | 简答 | 给出两种算法差异 |
| `task-innovation-edge` | 边缘计算负载均衡挑战 | 创新挑战 | kp-application-load-balancing | `ex-application-load-balancing-001` | 设计 | 提交架构图、指标和权衡说明 |
| `task-project-smart-home` | 智能家居网络设计 | 项目实践 | kp-network-design | `ex-network-design-001` | 设计 | 提交拓扑、地址规划和安全边界 |
| `task-troubleshooting-dns` | DNS 解析异常排错 | 排错 | kp-application-dns | `ex-application-dns-001` | 简答 | 给出根因、证据和修复步骤 |

六类任务全部关联试题，且任务与其试题落在同一知识点上——
这两项由 `apps/api/tests/test_exercises.py` 自动校验，不依赖人工核对。
